...
function addTask(event) {
  ...
  li.innerHTML = `
      ...
      <button class="deleteButton" onclick="deleteTask('${taskId}')" title="Deletar tarefa">❌</button>
  `;
...
function deleteTask(taskId) {
  ...
  const taskList = document.querySelector('#taskList');
  const li = document.querySelector(`#${taskId}`);
  ...
  taskList.innerHTML = tasks
    .map(
      ...
      <li id="${task.id}">
        ...
        <button class="deleteButton" onclick="deleteTask('${task.id}')" title="Deletar tarefa">❌</button>
      </li>
    `
    )
    .join('');
